...
const CString EFFECT_TEXTS[] = {
    // // //
    _T(""),
    _T("Fxx - Set speed to XX, cancels groove. If xx>=10, tempo must be ")
    _T("fixed."),
    ...
    _T("H0y - 5B envelope shape, bit 3 = Continue, bit 2 = Attack, bit 1 = ")
    _T("Alternate, bit 0 = Hold"),
    ...
    _T("Txy - Delayed transpose (upward), X = frames to wait, Y = semitone ")
    _T("offset"),
    _T("Txy - Delayed transpose (downward), X - 8 = frames to wait, Y = ")
    _T("semitone offset"),
...
CFamiTrackerView::CFamiTrackerView()
    : mClipboardFormat(0), m_iInsertKeyStepping(1), m_bEditEnable(false),
      m_bMaskInstrument(false), m_bMaskVolume(true),
      m_bSwitchToInstrument(false), m_iPastePos(PASTE_CURSOR), // // //
      m_iLastNote(NONE),                                       // // //
      m_iLastVolume(MAX_VOLUME), m_iLastInstrument(0),
      m_iLastEffect(EF_NONE), // // //
      m_iLastEffectParam(0),  // // //
      m_iSwitchToInstrument(-1), m_bFollowMode(true),
      m_bCompactMode(false), // // //
      m_iMarkerFrame(-1),    // // // 050B
      m_iMarkerRow(-1),      // // // 050B
      m_iAutoArpNotes(),     // // //
      m_iAutoArpPtr(0), m_iLastAutoArpPtr(0), m_iAutoArpKeyCount(0),
      m_iSplitNote(-1),                    // // //
      m_iSplitChannel(-1),                 // // //
      ...
      m_iMenuChannel(-1), m_iKeyboardNote(-1), m_nDropEffect(DROPEFFECT_NONE),
      m_bDragSource(false), m_pPatternEditor(new CPatternEditor()) {
...
#ifdef _DEBUG
void CFamiTrackerView::AssertValid() const { CView::AssertValid(); }

void CFamiTrackerView::Dump(CDumpContext &dc) const { CView::Dump(dc); }

CFamiTrackerDoc *
CFamiTrackerView::GetDocument() const // non-debug version is inline
{
...
void CFamiTrackerView::OnDraw(CDC *pDC) {
  ...
  if (!pDoc->IsFileLoaded()) {
    ...
    pDC->DrawText(str, _tcslen(str), &textRect,
...
void CFamiTrackerView::OnVScroll(UINT nSBCode, UINT nPos,
...
void CFamiTrackerView::OnHScroll(UINT nSBCode, UINT nPos,
                                 CScrollBar *pScrollBar) {
  CFamiTrackerDoc *pDoc = GetDocument();
  ASSERT_VALID(pDoc);
...
void CFamiTrackerView::OnRButtonUp(UINT nFlags, CPoint point) {
  ...
  if (m_pPatternEditor->IsOverHeader(point)) {
    ...
    pPopupMenu->EnableMenuItem(ID_TRACKER_RECORDTOINST,
                               theApp.IsPlaying() ? MF_DISABLED
                                                  : MF_ENABLED); // // //
    pPopupMenu->EnableMenuItem(ID_TRACKER_RECORDERSETTINGS,
                               theApp.IsPlaying() ? MF_DISABLED
                                                  : MF_ENABLED); // // //
    CMenu *pMeterMenu = pPopupMenu->GetSubMenu(6);               // // // 050B
    int Rate = theApp.GetSoundGenerator()->GetMeterDecayRate();
    pMeterMenu->CheckMenuItem(Rate == DECAY_FAST ? ID_DECAY_FAST
                                                 : ID_DECAY_SLOW,
                              MF_CHECKED | MF_BYCOMMAND);
    pPopupMenu->TrackPopupMenu(TPM_RIGHTBUTTON, point.x + WinRect.left,
                               point.y + WinRect.top, this);
    ...
    pPopupMenu->TrackPopupMenu(TPM_RIGHTBUTTON, point.x + WinRect.left,
                               point.y + WinRect.top, GetParentFrame());
  }
...
void CFamiTrackerView::OnLButtonDblClk(UINT nFlags, CPoint point) {
  if (theApp.GetSettings()->General.bDblClickSelect &&
      !m_pPatternEditor->IsOverHeader(point))
    return;
...
void CFamiTrackerView::OnXButtonDown(UINT nFlags, UINT nButton,
                                     CPoint point) // // //
{
  /*
...
BOOL CFamiTrackerView::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt) {
  ...
  if (bControlPressed && bShiftPressed) {
    ...
    if (!(theApp.IsPlaying() && !IsSelecting() && m_bFollowMode)) // // //
      pAction = new CPActionTranspose{
          zDelta > 0 ? TRANSPOSE_INC_NOTES : TRANSPOSE_DEC_NOTES}; // // //
...
void CFamiTrackerView::OnEditCopy() {
  if (m_pPatternEditor->GetSelectionCondition() ==
      SEL_NONTERMINAL_SKIP) { // // //
    CMainFrame *pMainFrm = static_cast<CMainFrame *>(GetParentFrame());
    ASSERT_VALID(pMainFrm);
  ...
  std::shared_ptr<CPatternClipData> pClipData(
      m_pPatternEditor->Copy()); // // //
...
void CFamiTrackerView::OnEditPasteMix() // // //
{
  ...
  // Add an undo point
  CPatternAction *pAction =
      new CPatternAction(CPatternAction::ACT_EDIT_PASTE); // // //
  pAction->SetPaste(pClipData);
...
void CFamiTrackerView::OnTrackerDetune() // // //
{
  ...
  pDoc->SetTuning(DetuneDlg.GetDetuneSemitone(),
                  DetuneDlg.GetDetuneCent()); // // // 050B
...
void CFamiTrackerView::OnEditVolumeMask() { m_bMaskVolume = !m_bMaskVolume; }
...
void CFamiTrackerView::OnTrackerPlayrow() {
  ...
  const int Track =
      static_cast<CMainFrame *>(GetParentFrame())->GetSelectedTrack();
...
void CFamiTrackerView::OnInitialUpdate() {
  ...
  pMainFrame->SetSongInfo(pDoc->GetSongName(), pDoc->GetSongArtist(),
  ...
  theApp.GetSoundGenerator()->SetMeterDecayRate(
      theApp.GetSettings()->MeterDecayRate);                   // // // 050B
  theApp.GetSoundGenerator()->DocumentPropertiesChanged(pDoc); // // //
...
void CFamiTrackerView::OnUpdate(CView * /*pSender*/, LPARAM lHint,
                                CObject * /*pHint*/) {
  ...
  case UPDATE_PROPERTIES:
    ...
    m_pPatternEditor
        ->InvalidateBackground(); // Channel count might have changed,
                                  // recalculated pattern layout
    m_pPatternEditor->InvalidatePatternData();
    m_pPatternEditor->InvalidateHeader();
    RedrawPatternEditor();
    UpdateNoteQueues(); // // //
...
void CFamiTrackerView::OnEditPasteSpecialFill() { m_iPastePos = PASTE_FILL; }

void CFamiTrackerView::OnUpdatePasteSpecial(CCmdUI *pCmdUI) {
  if (pCmdUI->m_pMenu != NULL)
    pCmdUI->m_pMenu->CheckMenuRadioItem(
        ID_PASTESPECIAL_CURSOR, ID_PASTESPECIAL_FILL,
        ID_PASTESPECIAL_CURSOR + m_iPastePos, MF_BYCOMMAND);
}
...
bool CFamiTrackerView::PlayerGetNote(int Track, int Frame, int Channel, int Row,
                                     stChanNote &NoteData) {
  CFamiTrackerDoc *pDoc = GetDocument();
...
void CFamiTrackerView::PlayerPlayNote(int Channel, stChanNote *pNote) {
  ...
  if (pNote->Instrument < MAX_INSTRUMENTS && pNote->Note > 0 &&
      Channel == m_pPatternEditor->GetChannel() && m_bSwitchToInstrument) {
...
bool CFamiTrackerView::GetFollowMode() const { return m_bFollowMode; }
...
void CFamiTrackerView::OnBookmarksNext() {
  ...
  ASSERT_VALID(pDoc);
  CMainFrame *pMainFrame = static_cast<CMainFrame *>(GetParentFrame());
  CBookmarkCollection *pCol =
      pDoc->GetBookmarkManager()->GetCollection(pMainFrame->GetSelectedTrack());
  ASSERT(pCol);
  ...
  if (CBookmark *pMark = pCol->FindNext(GetSelectedFrame(), GetSelectedRow())) {
    ...
    if (pMark->m_Highlight.First != -1)
      str1.Format(_T("%i"), pMark->m_Highlight.First);
    CString str2 = _T("None");
    if (pMark->m_Highlight.Second != -1)
      str2.Format(_T("%i"), pMark->m_Highlight.Second);
    CString Text;
    AfxFormatString3(Text, IDS_BOOKMARK_FORMAT, pMark->m_sName.c_str(), str1,
                     str2);
...
void CFamiTrackerView::OnBookmarksPrevious() {
  ...
  if (CBookmark *pMark =
          pCol->FindPrevious(GetSelectedFrame(), GetSelectedRow())) {
    SelectFrame(pMark->m_iFrame);
    SelectRow(pMark->m_iRow);
    CString str1 = _T("None");
    if (pMark->m_Highlight.First != -1)
...
void CFamiTrackerView::SetChannelMute(int Channel, bool bMute) {
  ...
  if (bMute && pDoc->GetChannelType(Channel) ==
                   theApp.GetSoundGenerator()->GetRecordChannel())
...
void CFamiTrackerView::SetInstrument(int Instrument) {
  ...
  m_iLastInstrument =
      GetInstrument(); // Gets actual selected instrument //  Instrument;
}
...
void CFamiTrackerView::InsertNote(int Note, int Octave, int Channel,
                                  int Velocity) {
  // Inserts a note
  const int Track =
      static_cast<CMainFrame *>(GetParentFrame())->GetSelectedTrack();
  ...
  if (m_bEditEnable) {
    ...
    if (AddAction(pAction)) {
      ...
      if (m_pPatternEditor->GetColumn() == C_NOTE && !theApp.IsPlaying() &&
          m_iInsertKeyStepping > 0 && !pSettings->Midi.bMidiMasterSync) {
        StepDown();
        pAction->SaveRedoState(
...
void CFamiTrackerView::PlayNote(unsigned int Channel, unsigned int Note,
                                unsigned int Octave,
  ...
  */
  ...
  if (Channel < static_cast<unsigned>(pDoc->GetChannelCount())) {
    int ret = pDoc->GetChannelIndex(
        m_pNoteQueue->Trigger(MidiNote, pDoc->GetChannelType(Channel)));
    if (ret != -1) {
  ...
  if (theApp.GetSettings()->General.bPreviewFullRow) {
    ...
    for (int i = 0; i < Channels; ++i) {
      ...
      if (!m_bMuteChannels[i] && i != Channel)
        theApp.GetSoundGenerator()->QueueNote(
            i, ChanNote, (i == Channel) ? NOTE_PRIO_2 : NOTE_PRIO_1);
    }
  }
}

void CFamiTrackerView::ReleaseNote(unsigned int Channel, unsigned int Note,
...
void CFamiTrackerView::HaltNote(unsigned int Channel, unsigned int Note,
                                unsigned int Octave) const {
  // Halts a channel
  stChanNote NoteData{}; // // //
...
void CFamiTrackerView::HaltNoteSingle(unsigned int Channel) const {
  ...
  if (Channel < static_cast<unsigned>(pDoc->GetChannelCount())) {
    for (const auto &i :
         m_pNoteQueue->StopChannel(pDoc->GetChannelType(Channel))) {
...
/*
static void FixNoise(unsigned int &MidiNote, unsigned int &Octave, unsigned int
&Note)
{
        // NES noise channel
...
// Play a note
void CFamiTrackerView::TriggerMIDINote(unsigned int Channel,
                                       unsigned int MidiNote,
                                       unsigned int Velocity, bool Insert) {
  CFamiTrackerDoc *pDoc = GetDocument();

  if (MidiNote >= NOTE_COUNT)
    MidiNote = NOTE_COUNT - 1; // // //
  ...
  TRACE("%i: Trigger note %i on channel %i\n", GetTickCount(), MidiNote,
        Channel);
}

// Cut the currently playing note
void CFamiTrackerView::CutMIDINote(unsigned int Channel, unsigned int MidiNote,
                                   bool InsertCut) {
  CFamiTrackerDoc *pDoc = GetDocument();
...
void CFamiTrackerView::ReleaseMIDINote(unsigned int Channel,
                                       unsigned int MidiNote, bool InsertCut) {
  CFamiTrackerDoc *pDoc = GetDocument();
...
void CFamiTrackerView::UpdateNoteQueues() // // //
{
  ...
  if (m_bEditEnable)
    ...
    if (pDoc->ExpansionEnabled(SNDCHIP_MMC5))
      m_pNoteQueue->AddMap({CHANID_SQUARE1, CHANID_SQUARE2, CHANID_MMC5_SQUARE1,
                            CHANID_MMC5_SQUARE2});
    else
...
void CFamiTrackerView::OnKeyDown(UINT key, UINT nRepCnt, UINT nFlags) {
  ...
  if ((key == VK_ADD || key == VK_SUBTRACT) &&
      theApp.GetSettings()->General.bHexKeypad) // // //
    HandleKeyboardInput(Keycode(key));
  else if (!theApp.GetSettings()->General.bHexKeypad ||
           !(key == VK_RETURN && !(nFlags & KF_EXTENDED)))
    switch (key) {
    case VK_UP:
      OnKeyDirUp();
      break;
    ...
    case VK_F2:
      pFrame->SelectOctave(0);
      break;
    case VK_F3:
      pFrame->SelectOctave(1);
      break;
    case VK_F4:
      pFrame->SelectOctave(2);
      break;
    case VK_F5:
      pFrame->SelectOctave(3);
      break;
    case VK_F6:
      pFrame->SelectOctave(4);
      break;
    case VK_F7:
      pFrame->SelectOctave(5);
      break;
    case VK_F8:
      pFrame->SelectOctave(6);
      break;
    case VK_F9:
      pFrame->SelectOctave(7);
      break;
...
template <typename T> bool isAlphanumeric(T x) {
  auto num = static_cast<int>(x);
  return withinInclusive(num, 'A', 'Z') || withinInclusive(num, 'a', 'z') ||
         withinInclusive(num, '0', '9');
}

// OnChar operates on translated characters, and inserts non-alphanumeric
// effects.
...
bool CFamiTrackerView::EditInstrumentColumn(stChanNote &Note, Keycode Key,
                                            bool &StepDown, bool &MoveRight,
                                            bool &MoveLeft) {
  ...
  if (CheckClearKey(Key)) {
    SetInstrument(Note.Instrument =
                      MAX_INSTRUMENTS); // Indicate no instrument selected
  ...
  if (Note.Instrument == MAX_INSTRUMENTS ||
      Note.Instrument == HOLD_INSTRUMENT) // // // 050B
    Note.Instrument = 0;
...
bool CFamiTrackerView::EditVolumeColumn(stChanNote &Note, Keycode Key,
                                        bool &bStepDown) {
  int EditStyle = theApp.GetSettings()->General.iEditStyle;
...
bool CFamiTrackerView::EditEffNumberColumn(stChanNote &Note, Input input,
                                           int EffectIndex, bool &bStepDown) {
  int EditStyle = theApp.GetSettings()->General.iEditStyle;
  ...
  char effect; // TODO promote to int. So Unicode codepoints won't clash with
               // ASCII effects.
  ...
  if (auto p = get_if<Keycode>(&input)) {
    ...
    throw std::runtime_error("EditEffNumberColumn called with invalid Input "
                             "(neither Keycode nor Character)");
  }
...
bool CFamiTrackerView::EditEffParamColumn(stChanNote &Note, Keycode Key,
                                          int EffectIndex, bool &bStepDown,
                                          bool &bMoveRight, bool &bMoveLeft) {
  ...
  unsigned char Mask, Shift;
  if (Column == C_EFF1_PARAM1 || Column == C_EFF2_PARAM1 ||
      Column == C_EFF3_PARAM1 || Column == C_EFF4_PARAM1) {
    Mask = 0x0F;
    Shift = 4;
  } else {
    Mask = 0xF0;
  ...
  switch (EditStyle) {
  case EDIT_STYLE_FT2: // FT2
    Note.EffParam[EffectIndex] =
        (Note.EffParam[EffectIndex] & Mask) | Value << Shift;
    bStepDown = true;
    break;
  case EDIT_STYLE_MPT: // Modplug
    Note.EffParam[EffectIndex] =
        ((Note.EffParam[EffectIndex] & 0x0F) << 4) | Value & 0x0F;
    break;
  case EDIT_STYLE_IT: // IT
    Note.EffParam[EffectIndex] =
        (Note.EffParam[EffectIndex] & Mask) | Value << Shift;
...
void CFamiTrackerView::HandleKeyboardInput(Input input) {
  ...
  if (auto p = get_if<Keycode>(&input)) {
    ...
    case C_NOTE:
      if (CheckRepeatKey(key)) {
        if (m_iLastNote == 0) {
          ...
          Note.Note = RELEASE;
        } else if (m_iLastNote >= NOTE_ECHO &&
                   m_iLastNote <= NOTE_ECHO + ECHO_BUFFER_LENGTH) { // // //
          Note.Note = ECHO;
        ...
        Note.Note = ECHO;
        Note.Octave = static_cast<CMainFrame *>(GetParentFrame())
                          ->GetSelectedOctave(); // // //
    ...
    case C_INSTRUMENT1:
    case C_INSTRUMENT2:
      if (!EditInstrumentColumn(Note, key, bStepDown, bMoveRight,
                                bMoveLeft)) // invert test?
        return;
      break;
    // Volume column
    case C_VOLUME:
    ...
    case C_EFF1_PARAM1:
    case C_EFF1_PARAM2:
      if (!EditEffParamColumn(Note, key, Index, bStepDown, bMoveRight,
                              bMoveLeft))
        return;
      break;
    }
...
void CFamiTrackerView::SplitAdjustChannel(unsigned int &Channel,
                                          const stChanNote &Note) const // // //
{
...
int CFamiTrackerView::TranslateKeyModplug(Keycode Key) const {
  ...
  int Track = static_cast<CMainFrame *>(GetParentFrame())->GetSelectedTrack();
  int Octave = static_cast<CMainFrame *>(GetParentFrame())
                   ->GetSelectedOctave(); // // // 050B

  stChanNote NoteData;
  pDoc->GetNoteData(Track, m_pPatternEditor->GetFrame(),
                    m_pPatternEditor->GetChannel(), m_pPatternEditor->GetRow(),
                    &NoteData);
  ...
  if (m_bEditEnable && Key >= '0' && Key <= '9') { // // //
    ...
    if (KeyOctave < 0)
      KeyOctave += 10;
    if (KeyOctave >= OCTAVE_RANGE)
      KeyOctave = OCTAVE_RANGE - 1;
...
int CFamiTrackerView::TranslateKeyDefault(Keycode Key) const {
  // Default conversion
  int KeyNote = 0;
  int KeyOctave = static_cast<CMainFrame *>(GetParentFrame())
                      ->GetSelectedOctave(); // // // 050B
  ...
  while (pMIDI->ReadMessage(Message, Channel, Data1, Data2)) {
    ...
    case MIDI_MSG_NOTE_ON:
      ...
      AfxFormatString3(Status, IDS_MIDI_MESSAGE_ON_FORMAT,
                       MakeIntString(Data1 % 12), MakeIntString(Data1 / 12),
                       MakeIntString(Data2));
      break;
...
void CFamiTrackerView::OnTrackerUnmuteAllChannels() { UnmuteAllChannels(); }
...
void CFamiTrackerView::OnTrackerRecordToInst() // // //
{
  ...
  if (Channel == CHANID_DPCM || Chip == SNDCHIP_VRC7) {
    AfxMessageBox(IDS_DUMP_NOT_SUPPORTED, MB_ICONERROR);
    return;
  }
  ...
  theApp.GetSoundGenerator()->SetRecordChannel(
      Channel == theApp.GetSoundGenerator()->GetRecordChannel() ? -1 : Channel);
  InvalidateHeader();
}
...
void CFamiTrackerView::OnPickupRow() {
  ...
  case ECHO:
    ...
    break;
  default:
    m_iLastNote = (Note.Note - 1) + Note.Octave * 12;
  }
...
DROPEFFECT CFamiTrackerView::OnDragEnter(COleDataObject *pDataObject,
  ...
  if (m_pPatternEditor->GetSelectionCondition() ==
    ...
    MessageBeep(MB_ICONWARNING);
    static_cast<CMainFrame *>(GetParentFrame())
        ->SetMessageText(IDS_SEL_NONTERMINAL_SKIP);
    m_nDropEffect = DROPEFFECT_NONE;
  } else if (m_pPatternEditor->GetSelectionCondition() ==
             SEL_REPEATED_ROW) { // // //
    MessageBeep(MB_ICONWARNING);
    static_cast<CMainFrame *>(GetParentFrame())
        ->SetMessageText(IDS_SEL_REPEATED_ROW);
    m_nDropEffect = DROPEFFECT_NONE;
    ...
    // Get drag rectangle
    std::shared_ptr<CPatternClipData> pDragData(
        new CPatternClipData()); // // //
...
DROPEFFECT CFamiTrackerView::OnDragOver(COleDataObject *pDataObject,
                                        DWORD dwKeyState, CPoint point) {
...
BOOL CFamiTrackerView::OnDrop(COleDataObject *pDataObject,
...
bool CFamiTrackerView::IsDragging() const { return m_bDragSource; }
...
{
  int Channel = static_cast<CFamiTrackerDoc *>(m_pDocument)
                    ->GetChannelType(m_pPatternEditor->GetChannel());
  GetParentFrame()->SetMessageText(
      theApp.GetSoundGenerator()->RecallChannelState(Channel));
}

CString CFamiTrackerView::GetEffectHint(const stChanNote &Note,
                                        int Column) const // // //
{
  int Index = Note.EffNumber[Column];
  int Param = Note.EffParam[Column];
  if (Index >= EF_COUNT)
    return _T("Undefined effect");

  int Channel = m_pPatternEditor->GetChannel();
  int Chip = GetDocument()->GetChannel(Channel)->GetChip();
  if (Index > EF_FDS_VOLUME || (Index == EF_FDS_VOLUME && Param >= 0x40))
    ++Index;
  if (Index > EF_TRANSPOSE || (Index == EF_TRANSPOSE && Param >= 0x80))
    ++Index;
  if (Index > EF_SUNSOFT_ENV_TYPE ||
      (Index == EF_SUNSOFT_ENV_TYPE && Param >= 0x10))
    ++Index;
  if (Index > EF_FDS_MOD_SPEED_HI ||
      (Index == EF_FDS_MOD_SPEED_HI && Param >= 0x10))
    ++Index;
  if (Index > EF_FDS_MOD_DEPTH || (Index == EF_FDS_MOD_DEPTH && Param >= 0x80))
    ++Index;
  if (Index > EF_NOTE_CUT ||
      (Index == EF_NOTE_CUT && Param >= 0x80 && Channel == CHANID_TRIANGLE))
    ++Index;
  if (Index > EF_DUTY_CYCLE || (Index == EF_DUTY_CYCLE &&
                                (Chip == SNDCHIP_VRC7 || Chip == SNDCHIP_N163)))
    ++Index;
  if (Index > EF_DUTY_CYCLE || (Index == EF_DUTY_CYCLE && Chip == SNDCHIP_N163))
    ++Index;
  if (Index > EF_VOLUME || (Index == EF_VOLUME && Param >= 0xE0))
    ++Index;
  if (Index > EF_SPEED ||
      (Index == EF_SPEED && Param >= GetDocument()->GetSpeedSplitPoint()))
    ++Index;
