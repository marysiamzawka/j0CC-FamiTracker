...
const TCHAR FT_SHARED_MUTEX_NAME[] =
    _T("FamiTrackerMutex"); // Name of global mutex
const TCHAR FT_SHARED_MEM_NAME[] =
    _T("FamiTrackerWnd"); // Name of global memory area
...
CFamiTrackerApp::CFamiTrackerApp()
    : m_bThemeActive(false), m_pMIDI(NULL), m_pAccel(NULL), m_pSettings(NULL),
      m_pSoundGenerator(NULL), m_pChannelMap(NULL), m_customExporters(NULL),
      m_hWndMapFile(NULL),
...
BOOL CFamiTrackerApp::InitInstance() {
  ...
  LoadStdProfileSettings(
      MAX_RECENT_FILES); // Load standard INI file options (including MRU)

  // Load program settings
  m_pSettings = CSettings::GetObject();
  m_pSettings->LoadSettings();

  // Parse command line for standard shell commands, DDE, file open + some
  // custom ones
  ...
  CDocTemplate0CC *pDocTemplate = new CDocTemplate0CC( // // //
      IDR_MAINFRAME, RUNTIME_CLASS(CFamiTrackerDoc), RUNTIME_CLASS(CMainFrame),
  ...
  if (IsWindowsVistaOrGreater()) { // // //
    HKEY HKCU;
    long res_reg =
        ::RegOpenKey(HKEY_CURRENT_USER, _T("Software\\Classes"), &HKCU);
  ...
  AfxRegSetValue(HKEY_CLASSES_ROOT, "0CCFamiTracker.Document", REG_SZ,
                 FILE_ASSOC_NAME, lstrlen(FILE_ASSOC_NAME) * sizeof(TCHAR));
  ...
  if (pDocTemplate->GetDocString(strFileTypeId, CDocTemplate::regFileTypeId) &&
    ...
    strTemp.Format(_T("%s\\shell\\play\\%s"), (LPCTSTR)strFileTypeId,
                   _T("command"));
    AfxRegSetValue(HKEY_CLASSES_ROOT, strTemp, REG_SZ, strOpenCommandLine,
                   lstrlen(strOpenCommandLine) * sizeof(TCHAR));
  }
#endif
  ...
  if (cmdInfo.m_bExport) {
    ...
    exporter.CommandLineExport(cmdInfo.m_strFileName, cmdInfo.m_strExportFile,
                               cmdInfo.m_strExportLogFile,
...
#ifndef _DEBUG
  m_pMainWnd->GetMenu()->GetSubMenu(4)->RemoveMenu(ID_MODULE_CHANNELS,
                                                   MF_BYCOMMAND); // // //
#endif
...
int CFamiTrackerApp::ExitInstance() {
  // Close program
  // The document is already closed at this point (and detached from sound
  // player)
...
bool CFamiTrackerApp::IsThemeActive() const { return m_bThemeActive; }

bool GetFileVersion(LPCTSTR Filename, WORD &Major, WORD &Minor, WORD &Revision,
                    WORD &Build) {
  DWORD Handle;
...
void CFamiTrackerApp::LoadLocalization() {
  ...
  if (GetFileVersion(DLL_NAME, Major, Minor, Revision, Build)) {
    if (Major != VERSION_API || Minor != VERSION_MAJ ||
        Revision != VERSION_MIN || Build != VERSION_REV) // // //
      return;
...
{
  int confirm = AfxMessageBox(IDS_CLEAR_RECENTS,
                              MB_YESNO | MB_ICONEXCLAMATION | MB_DEFBUTTON2);
  if (confirm == IDNO) {
    return;
  } else if (confirm != IDYES) {
    throw std::runtime_error(
        "Recent Files Clear message box returned invalid value");
  }

  SAFE_RELEASE(m_pRecentFileList);
  m_pRecentFileList = new CRecentFileList(0, _T("Recent File List"),
                                          _T("File%d"), MAX_RECENT_FILES);
...
void CFamiTrackerApp::ShutDownSynth() {
  ...
  // If thread was suspended then it will auto-terminate, because sound hasn't
  // been initialized

  // Wait for thread to exit
  DWORD dwResult =
      ::WaitForSingleObject(hThread, CSoundGen::AUDIO_TIMEOUT + 1000);
...
void CFamiTrackerApp::RegisterSingleInstance() {
  ...
  m_hWndMapFile = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE,
                                    0, SHARED_MEM_SIZE, FT_SHARED_MEM_NAME);

  if (m_hWndMapFile != NULL) {
    LPTSTR pBuf = (LPTSTR)MapViewOfFile(m_hWndMapFile, FILE_MAP_ALL_ACCESS, 0,
                                        0, SHARED_MEM_SIZE);
    if (pBuf != NULL) {
    ...
    try {
      if ((hOpen = InternetOpen(_T("0CC_FamiTracker"),
        ...
        for (const auto &i : j) {
          ...
          if (Ver[0] > VERSION_API ||
              Ver[0] == VERSION_API &&
                  (Ver[1] > VERSION_MAJ ||
                   Ver[1] == VERSION_MAJ &&
                       (Ver[2] > VERSION_MIN ||
                        Ver[2] == VERSION_MIN && Ver[3] > VERSION_REV))) {
            int Y = 1970, M = 1, D = 1;
            sscanf_s(i["published_at"].get<std::string>().c_str(), "%d-%d-%d",
                     &Y, &M, &D);
            ...
            m_pVersionMessage.Format(
                _T("A new version of " APP_NAME " is now available:\n\n"
                   "Version %d.%d.%d.%d (released %s %d, %d)\n\n%s\n\n"
                   "Pressing \"Yes\" will launch the Github web page for this "
                   "release."),
                Ver[0], Ver[1], Ver[2], Ver[3], MONTHS[--M], D, Y, desc);
            if (Start)
              m_pVersionMessage.Append(
                  _T(" (Version checking on startup may be disabled in the ")
                  _T("configuration menu.)"));
            m_pVersionURL.Format(
                _T("https://github.com/jimbo1qaz/0CC-FamiTracker/releases/tag/")
                _T("v%d.%d.%d.%d"),
      ...
      m_pVersionMessage =
          _T("Unable to get version information from the source repository.");
...
bool CFamiTrackerApp::CheckSingleInstance(CFTCommandLineInfo &cmdInfo) {
  ...
  if (GetLastError() == ERROR_ALREADY_EXISTS) {
    ...
    HANDLE hMapFile =
        OpenFileMapping(FILE_MAP_ALL_ACCESS, FALSE, FT_SHARED_MEM_NAME);
    if (hMapFile != NULL) {
      LPCTSTR pBuf = (LPTSTR)MapViewOfFile(hMapFile, FILE_MAP_ALL_ACCESS, 0, 0,
                                           SHARED_MEM_SIZE);
      if (pBuf != NULL) {
        // Get window handle
        ...
        if (hWnd != NULL) {
          ...
          ULONG_PTR result;
          SendMessageTimeout(hWnd, WM_COPYDATA, NULL, (LPARAM)&data,
...
int CFamiTrackerApp::GetCPUUsage() const {
  ...
  const HANDLE hThreads[THREAD_COUNT] = {m_hThread,
                                         m_pSoundGenerator->m_hThread};
  unsigned int TotalTime = 0;

  for (int i = 0; i < THREAD_COUNT; ++i) {
    FILETIME CreationTime, ExitTime, KernelTime, UserTime;
    GetThreadTimes(hThreads[i], &CreationTime, &ExitTime, &KernelTime,
                   &UserTime);
    TotalTime +=
        (KernelTime.dwLowDateTime - KernelLastTime[i].dwLowDateTime) / 1000;
    TotalTime +=
        (UserTime.dwLowDateTime - UserLastTime[i].dwLowDateTime) / 1000;
...
void CFamiTrackerApp::ThreadDisplayMessage(LPCTSTR lpszText, UINT nType,
                                           UINT nIDHelp) {
  m_pMainWnd->SendMessage(WM_USER_DISPLAY_MESSAGE_STRING, (WPARAM)lpszText,
                          (LPARAM)nType);
}

void CFamiTrackerApp::ThreadDisplayMessage(UINT nIDPrompt, UINT nType,
                                           UINT nIDHelp) {
  m_pMainWnd->SendMessage(WM_USER_DISPLAY_MESSAGE_ID, (WPARAM)nIDPrompt,
                          (LPARAM)nType);
}
...
void AfxFormatString3(CString &rString, UINT nIDS, LPCTSTR lpsz1, LPCTSTR lpsz2,
                      LPCTSTR lpsz3) {
...
CFTCommandLineInfo::CFTCommandLineInfo()
    : CCommandLineInfo(), m_bLog(false), m_bExport(false), m_bPlay(false),
      m_strExportFile(_T("")), m_strExportLogFile(_T("")),
      m_strExportDPCMFile(_T("")) {}

void CFTCommandLineInfo::ParseParam(const TCHAR *pszParam, BOOL bFlag,
                                    BOOL bLast) {
  if (bFlag) {
    ...
    // This is intended for a small helper program that avoids the problem with
    // console on win32 programs, and should remain undocumented. I'm using it
    // for testing.
...
CDocTemplate0CC::CDocTemplate0CC(UINT nIDResource, CRuntimeClass *pDocClass,
...
CDocTemplate::Confidence CDocTemplate0CC::MatchDocType(const char *pszPathName,
...
BOOL CDocManager0CC::DoPromptFileName(CString &fileName, UINT nIDSTitle,
  ...
  CFileDialog OpenFileDlg(
      bOpenFileDialog, _T("0cc"), NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
      _T(APP_NAME
         " modules (*.0cc;*.ftm)|*.0cc; *.ftm|All files (*.*)|*.*||"), // // //
      AfxGetMainWnd(), 0);
